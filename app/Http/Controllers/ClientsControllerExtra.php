...
class ClientsControllerExtra extends Controller
{
    ...
    {  
        ...
        return view('clients.filterClients',compact('countries','industries','clients','status','bdGrades','industryFilter','statusFilter','bd_gradeFilter','countryFilter','cityFilter'));                     
    }
...
public function filterShow(Request $request)
    {  
       ...
       if($request->bd_grade) 
       {
        $bd_gradeFilter=$request->bd_grade;
        $chosenGrade= DB::table('bd_grade')->where('id',$bd_gradeFilter)->first();
        $filterString = $filterString." AND clients.bd_grade='$bd_gradeFilter'";
       }
       ...
       $clients = Client::select(array('clients.*','Country.Name AS countryName', 'City.Name AS cityName', 'status.status AS currentStatus' , 'bd_grade.grade AS bdGrade','industries.name AS industryName',DB::raw('(SELECT count(id) FROM contacts WHERE contacts.client = clients.id) AS addedContacts'),DB::raw('(SELECT count(id) FROM clientCalls WHERE clientCalls.client = clients.id) AS addedCalls')))
        ...
        return view('clients.filterClients',compact('countries','industries','clients','status','bdGrades','industryFilter','statusFilter','bd_gradeFilter','countryFilter','cityFilter','chosenIndustry','chosenStatus','chosenGrade','chosenCountry','chosenCity'));                     
    }
